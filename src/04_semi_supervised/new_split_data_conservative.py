...
seed_ratio = 0.3        # 少量有标注
unlabeled_ratio = 0.7   # 模拟无标注
...
# ---------- 创建目录 ----------
dirs = [
    dataset_root / "seed-conservative/images/train",
    dataset_root / "seed-conservative/labels/train",
    dataset_root / "unlabeled-conservative/images/train",
    dataset_root / "unlabeled-conservative/labels_hidden/train",
]
...
for img_path in seed_files:
    ...
    shutil.copy(img_path, dataset_root / "seed-conservative/images/train" / img_path.name)
    shutil.copy(label_path, dataset_root / "seed-conservative/labels/train" / label_path.name)
...
for img_path in unlabeled_files:
    ...
    shutil.copy(img_path, dataset_root / "unlabeled-conservative/images/train" / img_path.name)

    # 标签隐藏（仅保存用于评估，不参与训练）
    shutil.copy(label_path, dataset_root / "unlabeled-conservative/labels_hidden/train" / label_path.name)
