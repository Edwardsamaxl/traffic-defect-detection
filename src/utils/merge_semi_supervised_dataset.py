...
def copy_pairs(pairs, merge_root, split):
...
def main():
    ...
    # ================== 收集所有样本 ==================
    image_label_pairs = []
    image_label_pairs.extend(collect_pairs(seed_root / "images", seed_root / "labels"))
    image_label_pairs.extend(
        collect_pairs(pseudo_root / "images", pseudo_root / "new-pseudo_labels")
    )

    print(f"Total samples collected: {len(image_label_pairs)}")
    if len(image_label_pairs) == 0:
        raise RuntimeError("No samples found, please check directory structure.")
    ...
    # ================== 拷贝 ==================
    copy_pairs(train_pairs, merge_root, "train")
    copy_pairs(val_pairs, merge_root, "val")
